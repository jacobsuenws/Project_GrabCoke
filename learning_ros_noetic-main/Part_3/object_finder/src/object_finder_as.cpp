...
bool ObjectFinder::find_toy_block(float surface_height, geometry_msgs::PoseStamped &object_pose) {
    ...
    double block_height = 0.15; //this height is specific to the COKE_CAN model
    //if insufficient points in plane, find_plane_fit returns "false"
    //should do more sanity testing on found_object status
    //hard-coded search bounds based on a COKE_CAN of width 0.05
    found_object = pclUtils_.find_plane_fit(0.3, 1, -0.5, 0.3, surface_height + 0.14, surface_height + 0.16, 0.001,
